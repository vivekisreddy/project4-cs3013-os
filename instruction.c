...
int Instruction_Load(int pid, int va) {
    char* physmem = Memsim_GetPhysMem(); // Ensure physmem is defined
    int pa = MMU_TranslateAddress(pid, VPN(va), PAGE_OFFSET(va));

    if (pa == -1) {
        int swapSlot = PT_GetSwapSlot(pid, VPN(va));
        if (swapSlot != -1) {
            // Bring page back into memory
            pa = Memsim_FirstFreePFN();
            if (pa == -1) pa = Memsim_EvictPage(); // Ensure function is declared

            FILE* swapFile = fopen("disk.txt", "r");
            if (swapFile) {
                fseek(swapFile, swapSlot * PAGE_SIZE, SEEK_SET);
                fread(&physmem[pa], PAGE_SIZE, 1, swapFile);
                fclose(swapFile);
            }
            PT_ClearSwapped(pid, VPN(va), PFN(pa));  // Ensure function is declared
        } else {
            printf("Error: Virtual address %d is not valid.\n", va);
            return 1;
        }
    }
    return pa;
